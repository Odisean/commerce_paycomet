...
class PaycometRedirectCheckoutForm extends BasePaymentOffsiteForm implements ContainerInjectionInterface {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state): array {
    ...
    $signature = hash('sha512',
      $gateway_settings['client_code']  .
      $gateway_settings['terminal'] .
      $operation .
      $ds_oder .
      $amount .
      $gateway_settings['currency'] .
      md5($gateway_settings['key']));
    ...
    $vhash = hash('sha512', md5($query . md5($gateway_settings['key'])));

    $data['VHASH'] = $vhash;
    $gateway_settings['url_iframe'] = 'https://api.paycomet.com/gateway/ifr-bankstore';
